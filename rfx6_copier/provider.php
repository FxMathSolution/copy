<?php
	...
	if (isset($_GET['provider'])) $provider = $_GET['provider']; else die('-1<br>invalid parameter format');
	...
	switch ($status)
	{
	  case $new_order:
		...
		$db = new db();
		$error = '';
		$id = $db->add_provider_deal($provider, $broker, $account_no, $ticket, $symbol, $type, $lot,
	  ...
	  case $close_order:
		...
		$db = new db();
		$error = '';
		$id = $db->close_provider_deal($provider, $broker, $account_no, $ticket, $close_price, $profit, $commission, $swap, $ip, $computer, $delay, $error);
		if ($id < 0) echo $id.'<br>'.$error;
		else echo $id.'<br>succeeded';
		break;
	  case $delete_order:
		$db = new db();
		$error = '';
		$id = $db->delete_provider_deal($provider, $broker, $account_no, $ticket, $ip, $computer, $delay, $error);
		if ($id < 0) echo $id.'<br>'.$error;
		else echo $id.'<br>succeeded';
		break;
	  case $modify_order:
		...
		$db = new db();
		$error = '';
		$id = $db->modify_provider_deal($provider, $broker, $account_no, $ticket, $open_price, $sl, $tp, $ip, $computer, $delay, $error);
		if ($id < 0) echo $id.'<br>'.$error;
		else echo $id.'<br>succeeded';
		break;
	  case $sub_order:
		...
		$db = new db();
		$error = '';
		$id = $db->suborder_provider_deal($provider, $broker, $account_no, $old_ticket, $ticket, $old_lot, $lot, $ip, $computer, $delay, $error);
		if ($id < 0) echo $id.'<br>'.$error;
		else echo $id.'<br>succeeded';
		break;
	}
